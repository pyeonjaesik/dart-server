...
var uploadmission = function(app){
  app.post('/uploadmission', upload.array('file',30), function(req, res, next) {
    ...
    if(database){
      var Ms = new database.MsModel({"userid":id,"user_id":_id,"text":text,"created_time":created_time,'post_id':post_id,clip:files,im:imagemode});
            ...
            database.PostModel.findOneAndUpdate({_id:post_id},{$inc:{cn:1}},(err,results)=>{
              ...
              if(results){
                ...
                database.AlarmModel.find({_id:user_id},(err,results)=>{
                  ...
                  if(results.length>0){
                    // let alarm=results[0]._doc.alarm;
                    // function checkalarm(em){
                    //   return em.type===3 && em.post_id===post_id;
                    // }
